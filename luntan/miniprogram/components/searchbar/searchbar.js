...
Component({
  ...
  lifetimes: {
    // @ts-ignore
    created(){
      luntanInfo.get().then(res=>{
        //console.log(res.data)
        this.data.titleInfo=res.data;
      })
    },
    attached(){
      // @ts-ignore
  ...
  methods: {
    ...
    search(){
      let search=this.data.event
      if(search){
        this.data.searchData=this.data.titleInfo.filter(function(obj){ 
          return( 
            String(obj.title)
            .indexOf(search)>-1
          )
      ...
      this.setData({
        list:this.data.searchData
      })

      console.log(this.data.searchData[0]._id)
